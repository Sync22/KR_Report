...
Web-view IA/performance pass (`2026-05-18 KST`, current contract updated `2026-08-31 KST`): the friend-facing `web-view` separates the top tabs by user task. `메인` focuses on the daily briefing and top-2 `오늘의 우선순위`. `관찰` holds the broader candidate pool; candidates below Top2 are explicitly outside the scheduled Top2 news-collection scope rather than labeled as a failed collection. `종목` holds selected-stock context, reports, stored price history, and an existing related-category drilldown when a mapping exists. `시장` distinguishes on-demand Toss queries from stored Toss snapshots. `순환매` holds reachable 업종/테마 detail, recent category trend, ETF trend, and the rotation overlay. ETF freshness and trend availability require actual stored Toss ETF rows. Existing KRX rows remain historical-review inputs and are not a required current-data dependency. Hidden tab data remains lazy-loaded where possible, and browser smoke must supplement layout checks with value QA for source/date/mapping truthfulness.
...
| Selected stock related context | `web-view` selected-stock DTO includes read-only `related_context` from stored same-date 업종/테마 rollups plus active `data/rotation_etf_candidates.json` mappings. ETF previews require an exact selected-date Toss ETF row; legacy KRX-derived mappings remain on hold instead of presenting an unavailable link. |
...
| Rotation overlay | `example/Cycle.jpg` is served with SVG overlay circles from `data/rotation_overlay_coordinates.json`; it is descriptive only. Highlighted sectors use same-date report summaries, and ETF previews require operator-managed active mappings plus exact-date stored Toss ETF rows. The legacy semiconductor/IT KRX-derived mappings are held because the current Toss snapshot lane does not contain those codes; unavailable ETF links are not presented as active functionality. |
