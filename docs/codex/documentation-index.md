...
Current work is the main-PC execution pass for the next-phase closeout, while excluding US market expansion, public trading recommendations, broad ingest, and automatic scheduling of the new market-briefing lane. Historical mini-PC provider/phone-review notes remain useful trace evidence, but the active main-PC readiness gates are separate: market-briefing manual review sends are `0/3`, external provider smoke is not recorded on this PC, KRX Open API rows for `2026-05-19` and `2026-05-20` are still unpublished in the latest probe batch, and the current-user `web-view` Startup shortcut still needs a real post-logon observation.
...
| Surface boundary | [surface-contract.md](/docs/codex/surface-contract.md) | `admin-gui` vs read-only `web-view` contract. |
