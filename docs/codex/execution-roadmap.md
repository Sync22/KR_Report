...
| External access candidates | [O] / watch | Keep the candidate set narrow: Tailscale for owner-only remote operation if still needed, Cloudflare Tunnel for friend-facing read-only `web-view` sharing. Historical mini-PC Cloudflare provider smoke for `https://web-view.example.invalid` is retained as trace evidence; the current main-PC provider smoke is not recorded. Keep access-code/allow-list controls enabled and keep `admin-gui` private. |
...
| Cloudflare Tunnel | Friend-facing URL candidate for the read-only `web-view` only. | Historical provider smoke passed for `https://web-view.example.invalid`, but current main-PC provider smoke is not recorded. Keep any provider mapped only to `{LOCAL_WEB_VIEW_TARGET}` or the chosen `web-view` loopback port and do not share `admin-gui`. |
