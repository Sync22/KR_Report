...
KRX investor-flow execution should now follow [krx-flow-execution-stages.md](details/krx/krx-flow-execution-stages.md).
...
P2 category/ETF execution is now summarized in this roadmap and [next-phase.md](next-phase.md).
...
Data source ownership and display naming are fixed in [data-source-policy.md](data-source-policy.md).
...
| KRX/ETF/flow | [krx-market-data-runbook.md](krx-market-data-runbook.md) |
| Admin GUI | [admin-gui-plan.md](admin-gui-plan.md) |
| Agent usage | [agent-guide.md](agent-guide.md) |
| 순환매 SVG overlay | [rotation-overlay-plan.md](rotation-overlay-plan.md) |
...
| Backtest observation / report reaction | [O] / watch | [backtest-observation-plan.md](plans/backtest-observation-plan.md) defines the read-only path. BO-1~BO-7 calculations/API/UI/initial QA exist; broader QA continues. |
| Scoring draft | Hold / archived | [scoring-draft-plan.md](plans/scoring-draft-plan.md) records historical research only. Hidden scoring CLIs are not active product commands and remain blocked from public output, product ordering, Telegram, and roadmap progress. |
...
The detailed boundary is fixed in [surface-contract.md](surface-contract.md):
...
| 6 | Start ETF/flow source study | Stage 4 done | [etf-flow-source-study.md](details/krx/etf-flow-source-study.md) fixes KRX-first and no-scoring boundaries. [krx-api-field-validation.md](details/krx/krx-api-field-validation.md) tracks Open API field capture. [krx-investor-flow-source-plan.md](details/krx/krx-investor-flow-source-plan.md) fixes Data Marketplace `[12008]`, `[12009]`, `[12010]` as screen-backed request candidates. `krx-flow-login-check` verifies raw `.env` login without DB writes, and Chrome login reminder remains fallback/debug only. |
...
| 10 | Draft mini PC migration handoff | Done | [mini-pc-migration-handoff.md](mini-pc-migration-handoff.md) captures zip/restore/verify/new-Codex briefing. |
