...
| --- | ---: | --- |
| Domestic MVP on the current main PC excluding broad public sharing and US expansion | 85-90% | Code, DB, and read-only preview foundations are usable, but this PC is not closeout-ready. The active blockers are manual market-briefing review sends `0/3`, real `2026-05-20` market-day scheduled-run observation, external provider smoke for the final shared URL, and post-logon observation that the local `web-view` Startup shortcut keeps `{LOCAL_WEB_VIEW_TARGET}` healthy. Same-day KRX latest-row publication/storage is no longer a blocker because official daily publication is next-business-day `08:00` KST. |
...
| Scheduler / operations | 75-80% | This main PC is still a desktop-validation host: default scheduler tasks are present but disabled, `StockMonitor-Shutdown` exists but is disabled, and `StockMonitor-WebViewHourlyRestart` is missing. Do not register new market-briefing or KRX probe scheduler tasks until read-only evidence and operator review justify it. |
| DB / KRX Open API baseline | 95% | `db-verify` is clean and snapshots cover through `2026-05-19`; stop same-day `2026-05-20` recurring probes because official publication is next-business-day `08:00` KST, and do not treat KRX OpenAPI as real-time intraday data. |
| User web-view | 88-92% | Stored-data `time_slot_mood_card` and value QA are working, but external provider smoke is not closed on this PC and Startup fallback still needs real post-logon health evidence. |
...
IA/performance note (`2026-05-18`): the page now has a dedicated `시장` tab. `메인` is kept for briefing and selected-date/selected-stock use; `관찰` owns `오늘의 관찰 후보` plus read-only reaction rows; `시장` owns KRX market reference and recent investor-flow trend; `ETF` owns ETF flow only; `순환매` keeps the rotation overlay. Hidden tab payloads are lazy-loaded, and the daily DTO no longer embeds the heavier `candidate_evidence` payload. Candidate cards render only user-facing `왜 눈에 띄는지` and `부족한 정보`, while raw `quality_flags` remain a DTO/testing/admin concern.
...
Historical mini-PC closeout note (`2026-05-19 01:25 KST`): the phone-readability gate was closed on that mini-PC DB. `market-briefing-readiness --recent-report-dates 5 --json` reported preview-ready `5/5`, manual review sends `3/3`, `phone_review_accepted=true`, and `schedule_candidate_ready=true` for the guarded `16:10~16:45` window. Latest backup `stock_monitor_{timestamp}_{tag}.db` was restore-smoked successfully, and `db-verify` remained clean. Elevated scheduler verification reported `StockMonitor-KrxDailyBackfill`, `StockMonitor-Notify`, `StockMonitor-Poll`, `StockMonitor-KrxMentionedFlowBackfill`, `StockMonitor-TelegramCommands`, and `StockMonitor-WebViewHourlyRestart` registered/enabled/Ready; `StockMonitor-Shutdown` remained absent. This is trace evidence only; the current main-PC gate is described in the next note and still has manual review sends `0/3`.

Main-PC continuation note (`2026-05-20 09:17 KST`, updated after official publication-window confirmation): the active goal is not complete. `next-phase-readiness --recent-report-dates 5 --stock-limit 20 --json` reports `completion_ready=false`, latest report date `2026-05-15`, market-briefing manual review sends `0/3`, `phone_review_accepted=false`, and missing KRX daily snapshots only for same-day `2026-05-20`. The latest unrestricted-network KRX availability probe changed `2026-05-19` at `2026-05-20T08:28:38+09:00` to mixed `partial=3` / `available=3`, reference date `2026-05-19`, raw rows `4011`, parsed rows `3701`; then a guarded live backfill at `09:17 KST` stored all six `2026-05-19` daily endpoints after backup `stock_monitor_{timestamp}_{tag}.db` and restore-smoke success. The stored deltas were ETF `874`, KOSPI stocks `923`, KOSDAQ stocks `1777`, KRX indices `36`, KOSPI indices `51`, and KOSDAQ indices `40`, with `incomplete_endpoints=0`; post-backfill `db-verify --json` remains clean with `operation_events=3008`, and `krx-baseline-analysis --json` reports stock/ETF/index tables through `2026-05-19`. Same-day `2026-05-20` remained `not_published` through later probes, which is expected because KRX Open API daily rows are officially published on the next Korean business day at `08:00` KST; the same-day probe automation was deleted. `market-briefing-readiness --recent-report-dates 5 --json` remains preview-ready for `5/5` dates with public-safe issue count `0`, but the phone-readability gate remains open with manual review sends `0/3`. `market-day-observation --date 2026-05-20 --json` at `2026-05-20T09:17:54+09:00` observes the new KRX daily backfill event for business date `2026-05-19`, but still marks `StockMonitor-TelegramCommands` (`08:05`), `StockMonitor-Notify` (`08:30`), and `StockMonitor-Poll` (`09:00`) as missing; the next market-day due check is `StockMonitor-KrxMentionedFlowBackfill` after `16:10 KST`. A current-user `{WEB_VIEW_STARTUP_SHORTCUT}` exists, but `web-view-startup-fallback-check --json` still reports local `/health` status `0`, so the gate remains blocked by real post-logon observation and runtime availability. Main-PC readiness command generation uses `{PYTHON_EXE}` across closeout and verification commands when `.venv\Scripts\python.exe` is absent. Non-elevated scheduler metadata checks still return `access_denied`, so Task Scheduler proof remains an elevated local-shell blocker and no task is registered or changed. The next KRX action is the next-business-day `08:10` scheduled backfill check, not same-day probing; the next market-day scheduled-run observation slot is `16:10 KST`.

Pre-09:00 refresh (`2026-05-20 08:36 KST`): read-only readiness commands were rerun before the next due slot. Poll remained `pending` because its `verify_after_at` is `09:00 KST`; TelegramCommands and Notify remained `missing`; `next-phase-readiness` stayed `completion_ready=false`; `db-verify` stayed clean with `operation_events=2998`; Startup fallback stayed blocked by `/health` status `0`; and non-elevated Task Scheduler metadata still returned `access_denied`. No Telegram send, scheduler registration/change, KRX snapshot write, broad ingest, public scoring, or recommendation output was performed. Keep the next follow-up anchored to `StockMonitor-Poll` after `09:00 KST`; run KRX availability probes only after the next meaningful slot around `09:28~09:34 KST`.

09:00 refresh (`2026-05-20 09:00 KST`): read-only readiness commands were rerun after the Poll verify time. Poll moved from `pending` to `missing`; TelegramCommands and Notify remain `missing`; `StockMonitor-KrxDailyBackfill` remains observed at `2026-05-20T01:00:58+09:00` for business date `2026-05-18`; and the next market-day scheduled-run observation is `StockMonitor-KrxMentionedFlowBackfill` after `16:10 KST`. `next-phase-readiness` stayed `completion_ready=false`; `db-verify` stayed clean with `operation_events=2998`; `market-briefing-readiness` stayed preview-ready `5/5`, public-safe issue count `0`, manual review sends `0/3`, and `phone_review_accepted=false`; Startup fallback stayed blocked by `/health` status `0`; and non-elevated Task Scheduler metadata still returned `access_denied`. No Telegram send, scheduler registration/change, KRX snapshot write, broad ingest, public scoring, or recommendation output was performed. Keep the next follow-up anchored to the KRX OpenAPI availability probe around `09:28~09:34 KST`; keep market-day observation open until the `16:10 KST` flow-backfill check is due.
...
| Report concentration | `리포트 집중`, `확인 후보` | mention count, broker breadth, target availability. |
...
| --- | --- | --- | --- |
| Report concentration | Convert raw report counts into concentration and breadth signals. | Same-date and 5-business-day report concentration by stock, broker breadth, target-price revision direction, and repeated-coverage labels. The first 5-business-day intensity and target-revision fields are implemented as stored factual context. | Do not treat more reports as automatically better. |
...
| --- | --- | --- |
| Report concentration scorecard | `reports`, `daily_stock_summaries` | `리포트 집중`, `반복 언급`, `브로커 폭` |
...
Latest-day retry note (`2026-05-17 08:17 KST`, updated `2026-05-19 01:25 KST`): after backup `data/backups/stock_monitor_{timestamp}_{tag}.db`, a bounded live `krx-backfill-missing daily --to-date 2026-05-15 --max-dates 1` retry filled all six daily Open API endpoints for `2026-05-15`: ETF 874 rows, stock 2701 rows, and index 127 rows, with `incomplete_endpoints=0`. A post-success backup `data/backups/stock_monitor_{timestamp}_{tag}.db` was created and restore-smoked successfully. For `2026-05-18`, backup `data/backups/stock_monitor_{timestamp}_{tag}.db` was created and a bounded live retry reached all six daily endpoints, but every endpoint returned `0` parsed rows and recorded `incomplete_endpoints=6`. The backup was restore-smoked successfully and DB verification remains clean. Retry later only after `db-verify`, a fresh backup if needed, and a bounded `krx-backfill-missing daily --to-date 2026-05-18 --max-dates 1`; do not fetch same-day `2026-05-19` pre-market rows.
