...
| Toss market context | Fixed Toss ranking and market-indicator investor-trading responses | `web-view` main-context GET projection and scheduled market briefing only | `KR / MARKET_TRADING_AMOUNT / realtime / Top20` is current market-attention context; Top2 overlap is display-only. `KOSPI`/`KOSDAQ` `1d` investor trading is prior-business-day aggregate context, not stock-level flow. No DB write, arbitrary query, or ordering change. |
