...
from stock_monitor.toss_openapi_preview import TossQuotePreviewProvider
...
def test_toss_quote_preview_concurrent_401s_reuse_the_single_reissued_token() -> None:
    ...
    provider = TossQuotePreviewProvider(
        config=config,
        endpoint=resolve_toss_readonly_endpoint("prices"),
        confirm_token_reissue=True,
...
def test_toss_quote_preview_waiter_does_not_receive_expired_stale_cache() -> None:
    config = TossOpenApiLabConfig(
        client_id="client-value",
        client_secret="secret-value",
        live_enabled=True,
...
def test_toss_quote_preview_waiter_allows_bounded_token_recovery_duration() -> None:
    config = TossOpenApiLabConfig(
        client_id="client-value",
        client_secret="secret-value",
        live_enabled=True,
