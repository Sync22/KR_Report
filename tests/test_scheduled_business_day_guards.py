...
class _MarketBriefingAllowedDateTime(datetime):
    @classmethod
    def now(cls, tz=None):
        return cls(2026, 5, 14, 16, 10, 0, tzinfo=tz)


class _MarketBriefingEarlyDateTime(datetime):
    @classmethod
    def now(cls, tz=None):
        return cls(2026, 5, 14, 15, 59, 0, tzinfo=tz)


class _MarketBriefingLateDateTime(datetime):
    @classmethod
    def now(cls, tz=None):
        return cls(2026, 5, 14, 16, 46, 0, tzinfo=tz)


class _MarketBriefingRunSuppressedDateTime(datetime):
    @classmethod
    def now(cls, tz=None):
        return cls(2026, 6, 2, 16, 10, 0, tzinfo=tz)
...
def test_scheduled_market_briefing_skips_run_suppressed_date(tmp_path, monkeypatch) -> None:
    monkeypatch.delenv("STOCK_MONITOR_DB_PATH", raising=False)
    monkeypatch.setenv("STOCK_MONITOR_RUN_SUPPRESSED_DATES", "2026-06-02")
    monkeypatch.setattr(cli_module, "datetime", _MarketBriefingRunSuppressedDateTime)
    config = RuntimeConfig.from_env(root_dir=tmp_path)
    repository = StockMonitorRepository(config.db_path, timezone=config.timezone)

    result = cli_module._run_scheduled_market_briefing(
        config,
        repository,
        dry_run=True,
        allow_repeat=False,
    ...
    assert event.component == "market-briefing"
    assert event.status == "skipped"
    assert "no-run date" in event.detail


def test_scheduled_market_briefing_dry_run_allows_window(tmp_path, monkeypatch, capsys) -> None:
    monkeypatch.delenv("STOCK_MONITOR_DB_PATH", raising=False)
    monkeypatch.setattr(cli_module, "datetime", _MarketBriefingAllowedDateTime)
    monkeypatch.setattr(cli_module, "_build_market_briefing_message", lambda *_args, **_kwargs: "market briefing preview")
    config = RuntimeConfig.from_env(root_dir=tmp_path)
    repository = StockMonitorRepository(config.db_path, timezone=config.timezone)
    ...
    assert result == 0
    assert "market briefing preview" in capsys.readouterr().out


def test_scheduled_market_briefing_skips_early_run_before_1600(tmp_path, monkeypatch) -> None:
    monkeypatch.delenv("STOCK_MONITOR_DB_PATH", raising=False)
    monkeypatch.setattr(cli_module, "datetime", _MarketBriefingEarlyDateTime)
    config = RuntimeConfig.from_env(root_dir=tmp_path)
    repository = StockMonitorRepository(config.db_path, timezone=config.timezone)
    ...
    event = repository.list_recent_operation_events(limit=1)[0]
    assert event.component == "market-briefing"
    assert event.event_type == "time-window"
    assert event.status == "skipped"


def test_scheduled_market_briefing_skips_late_run_after_1645(tmp_path, monkeypatch) -> None:
    monkeypatch.delenv("STOCK_MONITOR_DB_PATH", raising=False)
    monkeypatch.setattr(cli_module, "datetime", _MarketBriefingLateDateTime)
    config = RuntimeConfig.from_env(root_dir=tmp_path)
    repository = StockMonitorRepository(config.db_path, timezone=config.timezone)
    ...
    event = repository.list_recent_operation_events(limit=1)[0]
    assert event.component == "market-briefing"
    assert event.event_type == "late-run"
    assert event.status == "skipped"


def test_scheduled_market_briefing_allow_late_overrides_late_guard(tmp_path, monkeypatch, capsys) -> None:
    monkeypatch.delenv("STOCK_MONITOR_DB_PATH", raising=False)
    monkeypatch.setattr(cli_module, "datetime", _MarketBriefingLateDateTime)
    monkeypatch.setattr(cli_module, "_build_market_briefing_message", lambda *_args, **_kwargs: "late market briefing preview")
    config = RuntimeConfig.from_env(root_dir=tmp_path)
    repository = StockMonitorRepository(config.db_path, timezone=config.timezone)
    ...
    assert result == 0
    assert "late market briefing preview" in capsys.readouterr().out
